...
Germany}}\\\received{v3.3 released January 2014} }

\maketitle

\begin{abstract}
This guide is for authors who are preparing papers for the Taylor \& Francis journal {\it Journal of Statistical Computation and Simulation} ({\it gSCS})
using the \LaTeX\ document preparation system and the class file {\tt gSCS2e.cls}, which is available via the journal's homepage on the Taylor \& Francis website. Authors planning to submit papers
in \LaTeX\ are advised to use {\tt gSCS2e.cls} as early as possible in the creation of their files.

\begin{keywords}submission instructions; source file coding;
environments; references citation; fonts; numbering {\bf{(Please provide up to ten keywords taken
from terms used in your manuscript}})
\end{keywords}

\begin{classcode}F1.1; F4.3 {\bf{(... for example; authors are encouraged to provide two to six 2010 Mathematics Subject Classification codes)}}\end{classcode}
...
\hspace*{7pt} {1.1.}  The {\it gSCS} document class\\
\hspace*{7pt} {1.2.}  Submission of \LaTeX\ articles\\
\hspace*{24pt}        to the journal\\
{2.}    Using the {\it gSCS} class file\\
...
\hspace*{24pt} {4.10.1.}   References cited in the text  \\
\hspace*{24pt} {4.10.2.}   The list of references\\
\hspace*{7pt} {4.11.}   Appendices \\
\hspace*{7pt} {4.12.}   {\it gSCS} macros  \\
{5.}    Example of a section heading \\*
\hspace*{6pt}   including {\fontencoding{T1}\scshape{small caps}}, {\it italic}, and \\
\hspace*{6pt}   bold Greek such as ${\bm\kappa}$ \\
{6.}   {\em gSCS} journal style \\
\hspace*{10pt}{6.1.}   Spelling\\
\hspace*{10pt}{6.2.}   Hyphens, n-rules, m-rules and\\ \hspace*{27pt}minus signs\\
\hspace*{10pt}{6.3.}   References \\
\hspace*{10pt}{6.4.}   Maths fonts\\
{7.}   Troubleshooting\\
{8.}   Fixes for coding problems\\
{9.}   Obtaining the gSCS2e class file\\
...
In order to assist authors in the process of preparing a manuscript for the {\itshape Journal of Statistical Computation and Simulation} ({\it gSCS}), the journal's layout style has been implemented as a \LaTeXe\ class file based on the {\tt article} document class. A \textsc{Bib}\TeX\ style file is also provided to assist with the formatting of your references in a style appropriate to that of the journal.
...
The {\tt gSCSguide}.tex file can also be used as a template for composing an article for submission by cutting, pasting, inserting and
...
\subsection{The {\bi gSCS} document class}\label{S1.1}

The {\it gSCS} class file preserves the standard \LaTeXe\ interface such that any document that can
be produced using {\tt article.cls} can also be produced using the {\it gSCS} document class.
...
Manuscripts for possible publication in the journal should be submitted to the Editors for review as directed in the journal's Instructions for Authors, which may be found at {\tt{http://www.tandf.co.uk/journals/authors/gscsauth.asp}}.
...
\section{Using the {\bi gSCS} class file}

If the file {\tt gSCS2e.cls} is not already in the appropriate system directory for \LaTeXe\ files, either
arrange for it to be put there, or copy it to your working folder. In order to use the {\it gSCS} document class, replace the command
\verb"\documentclass{article}" at the beginning of your document with the command \verb"\documentclass{gSCS2e}".

The following document-class options should not be used with the {\it gSCS} class file:
%
\begin{itemize}
...
%
\begin{verbatim}
\title{{\itshape Journal of Statistical Computation and Simulation}
\break \LaTeX\ style guide for authors (Style 2 + NLM reference style)}
...
Francis journal {\em Journal of Statistical Computation and Simulation}
({\it gSCS}\,) using the \LaTeX\ document preparation system and the
class file {\tt gSCS2e.cls}, which is available via the journal's
homepage on the Taylor \& Francis website. Authors planning to submit
papers in \LaTeX\ are advised to use {\tt gSCS2e.cls} as early as
...
The {\it gSCS} class file provides numbered and unnumbered lists using the {\tt enumerate} environment and bulleted
...
the \verb"rotating" package, which is called by the \textit{gSCS} class file. The following commands can
...
The following notes are intended to help you achieve the best effects with the gSCS2e class file.


\subsection{Sections}

\LaTeXe\ provides five levels of section headings, all of which are defined in the gSCS2e class file:
\begin{enumerate}
   \item[(A)] \verb"\section"
...
The \textit{gSCS} class file will cope with most positioning of your illustrations and you should not normally need to use the optional placement specifiers of the \texttt{figure} environment. See
...
The former is called in the preamble of the \texttt{gSCSguide.tex} file (in order to allow your choice of alternative if preferred)
and the latter by the \texttt{gSCS2e} class file; both are included with the \LaTeX\ style guide package for this journal for convenience.
...
The \textit{gSCS} class file will cope with most positioning of your tables and you should not normally need to use the optional
placement specifiers of the \texttt{table} environment. The table caption appears above the body of the table in \textit{gSCS} style,
...
Tables produced using the {\tt booktabs} package of macros for typesetting tables are also compatible with the {\it gSCS} class file.
...
in the preamble of your .tex file before \verb"\begin{document}". Theorem-like structures in \textit{gSCS} are generally formatted as per the following examples:
...
\noindent These were defined as shown in detail in the preamble of the \texttt{gSCSguide}.tex file, and produced by typing, for example:
%
\begin{verbatim}
\begin{theorem}
More recent algorithms for solving the semidefinite programming
...
The {\it gSCS} class file will set displayed mathematics centred on the measure without equation numbers, provided
...
Upright lowercase Greek characters can be obtained with the \textit{gSCS} class file by inserting the letter `u' in the control
...
\noindent\verb"\bibliographystyle{gSCS}"
\newline\verb"\bibliography{gSCSguide}"
\vspace{12pt}

\noindent where the list of references should appear, where gSCS.bst is the name of the \textsc{Bib}\TeX\ style file for this journal and gSCSguide.bib is the database of bibliographic details for the references section included with the {\itshape gSCS} \LaTeX\ style guide package (to be replaced with the name of your own \textsc{Bib}\TeX\ database). The \LaTeX\ source file of your paper will extract from your .bib file only those references that are cited in that paper and list them in the References section of it.
...
\subsection{{\bi gSCS} macros}

Table~\ref{macros} gives a list of macros for use with {\it gSCS}. The list displays each macro's code and a
description/demonstration of its function.

\begin{table} \tbl{{\it gSCS} macros.}{\begin{tabular}{@{}ll}\toprule
...
\section{{\textit{gSCS}} journal style}
...
\begin{enumerate}
\item[(i)] Hyphens (one dash in \TeX/\LaTeX). {\it gSCS} uses hyphens for compound adjectives (e.g.\ low-density gas, least-squares fit,
...
\item[(iii)] The  m-rule (three dashes in \TeX/\LaTeX) has no specified use in {\it gSCS}.
...
or under headings or figures; figures/tables appearing out of order):
...
\]
\end{verbatim}
The \texttt{siunits} package of macros for typesetting units is also compatible with the \textit{gSCS} class file.
\end{enumerate}


\section{Obtaining the gSCS2e class file}\label{FTP}

\subsection{Via the Taylor \& Francis website}

This Guide for Authors and the gSCS2e.cls class file may be obtained via the Instructions for Authors
